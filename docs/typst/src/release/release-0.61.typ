...
#anchor("doc-release-release-0-61")
= Release 0.61

== Add support for table value constructors

Presto now supports the SQL table value constructor syntax to create inline tables. The #raw("VALUES") clause can be used anywhere a #raw("SELECT") statement is allowed. For example, as a top-level query:

#code-block(none, "VALUES ('a', 1), ('b', 2);")
...
Alternatively, in the #raw("FROM") clause:
...
#code-block("text", " letter | animal | letter |  fruit
--------+--------+--------+---------
 a      | ape    | a      | apple
 b      | bear   | b      | banana
(2 rows)")
...
- Add support for #raw("DECIMAL") type.
...
- Fix division by zero errors in variance aggregation functions \(#raw("VARIANCE"), #raw("STDDEV"), etc.\).
- Fix a bug when using #raw("DISTINCT") aggregations in the #raw("HAVING") clause.
- Fix an out of memory issue when writing large tables.
- Fix a bug when using #raw("ORDER BY rand()") in a #raw("JOIN") query.
