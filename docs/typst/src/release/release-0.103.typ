...
Use the configuration parameter #raw("task.default-concurrency") or the session property #raw("task_default_concurrency") to set the default number of parallel workers to use for join probes, hash builds and final aggregations. Additionally, the session properties #raw("task_join_concurrency"), #raw("task_hash_build_concurrency") and #raw("task_aggregation_concurrency") can be used to control the parallelism for each type of work.
...
- Removed the #raw("hive.max-split-iterator-threads") parameter and renamed #raw("hive.max-global-split-iterator-threads") to #raw("hive.max-split-iterator-threads").
...
- Add #link(label("fn-array-remove"), raw("array_remove")).
- Fix NPE in #link(label("fn-max-by"), raw("max_by")) and #link(label("fn-min-by"), raw("min_by")) caused when few rows were present in the aggregation.
- Reduce memory usage of #link(label("fn-map-agg"), raw("map_agg")).
