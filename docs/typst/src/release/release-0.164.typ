...
#anchor("doc-release-release-0-164")
= Release 0.164

== General

- Fix correctness issue for queries that perform #raw("DISTINCT") and #raw("LIMIT") on the results of a #raw("JOIN").
- Fix correctness issue when casting between maps where the key or value is the #raw("REAL") type.
- Fix correctness issue in #link(label("fn-min-by"), raw("min_by")) and #link(label("fn-max-by"), raw("max_by")) when nulls are present in the comparison column.
- Fail queries when #raw("FILTER") clause is specified for scalar functions.
...
- Fix query failure when #raw("TRY") or lambda expression with the exact same body is repeated in an expression.
- Fix split source resource leak in coordinator that can occur when a query fails.
- Improve #link(label("fn-array-join"), raw("array_join")) performance.
...
- Add #raw("VALIDATE") mode for #link(label("doc-sql-explain"))[EXPLAIN].
