...
#anchor("doc-release-release-0-158")
= Release 0.158
...
- Fix incorrect pruning of output columns in #raw("EXPLAIN ANALYZE").
- Fix ordering of #raw("CHAR") values so that trailing spaces are ordered after control characters.
...
- Fix case-sensitivity issues when operating on columns of #raw("ROW") data type.
- Fix failure when creating views for tables names that need quoting.
- Return #raw("NULL") from #link(label("fn-element-at"), raw("element_at")) for out-of-range indices instead of failing.
...
- Add experimental on-disk merge sort for aggregations. This can be enabled with the #raw("experimental.spill-enabled") configuration flag.
- Push down predicates for #raw("DECIMAL"), #raw("TINYINT"), #raw("SMALLINT") and #raw("REAL") data types.

== Hive

- Add hidden #raw("$bucket") column for bucketed tables that contains the bucket number for the current row.
