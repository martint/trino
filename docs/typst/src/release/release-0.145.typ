...
#anchor("doc-release-release-0-145")
= Release 0.145
...
- Fix compiler failure when #raw("TRY") is a sub-expression.
- Fix compiler failure when #raw("TRY") is called on a constant or an input reference.
- Add support for the #raw("integer") type to the Presto engine and the Hive, Raptor, Redis, Kafka, Cassandra and example-http connectors.
- Add initial support for the #raw("decimal") data type.
...
- Change verifier to only run read-only queries by default. This behavior can be changed with the #raw("control.query-types") and #raw("test.query-types") config flags.
...
- Add #raw("max_split_size") and #raw("max_initial_split_size") session properties to control the size of generated splits.
