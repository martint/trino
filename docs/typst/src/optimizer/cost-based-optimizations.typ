...
#anchor("doc-optimizer-cost-based-optimizations")
...
With cost-based join enumeration, Trino uses #link(label("doc-optimizer-statistics"))[Table statistics] provided by connectors to estimate the costs for different join orders and automatically picks the join order with the lowest computed costs.

The join enumeration strategy is governed by the #raw("join_reordering_strategy") #link(label("ref-session-properties-definition"))[session property], with the #raw("optimizer.join-reordering-strategy") configuration property providing the default value.

The possible values are:

- #raw("AUTOMATIC") \(default\) - enable full automatic join enumeration
- #raw("ELIMINATE_CROSS_JOINS") - eliminate unnecessary cross joins
- #raw("NONE") - purely syntactic join order

If you are using #raw("AUTOMATIC") join enumeration and statistics are not available or a cost can not be computed for any other reason, the #raw("ELIMINATE_CROSS_JOINS") strategy is used instead.
...
- Broadcast: each node participating in the query builds a hash table from all the data. The data is replicated to each node.
...
The join distribution strategy is governed by the #raw("join_distribution_type") session property, with the #raw("join-distribution-type") configuration property providing the default value.

The valid values are:

- #raw("AUTOMATIC") \(default\) - join distribution type is determined automatically for each join
- #raw("BROADCAST") - broadcast join distribution is used for all joins
- #raw("PARTITIONED") - partitioned join distribution is used for all join

=== Capping replicated table size

The join distribution type is automatically chosen when the join reordering strategy is set to #raw("AUTOMATIC") or when the join distribution type is set to #raw("AUTOMATIC"). In both cases, it is possible to cap the maximum size of the replicated table with the #raw("join-max-broadcast-table-size") configuration property or with the #raw("join_max_broadcast_table_size") session property. This allows you to improve cluster concurrency and prevent bad plans when the cost-based optimizer misestimates the size of the joined tables.
...
In order for the Trino optimizer to use the cost based strategies, the connector implementation must provide statistics.
