...
#anchor("ref-predicate-pushdown")

== Predicate pushdown

Predicate pushdown optimizes row-based filtering. It uses the inferred filter, typically resulting from a condition in a #raw("WHERE") clause to omit unnecessary rows. The processing is pushed down to the data source by the connector and then processed by the data source.

If predicate pushdown for a specific clause is successful, the #raw("EXPLAIN") plan for the query does not include a #raw("ScanFilterProject") operation for that clause.

#anchor("ref-projection-pushdown")

== Projection pushdown

Projection pushdown optimizes column-based filtering. It uses the columns specified in the #raw("SELECT") clause and other parts of the query to limit access to these columns. The processing is pushed down to the data source by the connector and then the data source only reads and returns the necessary columns.

If projection pushdown is successful, the #raw("EXPLAIN") plan for the query only accesses the relevant columns in the #raw("Layout") of the #raw("TableScan") operation.

#anchor("ref-dereference-pushdown")

== Dereference pushdown

Projection pushdown and dereference pushdown limit access to relevant columns, except dereference pushdown is more selective. It limits access to only read the specified fields within a top level or nested #raw("ROW") data type.

For example, consider a table in the Hive connector that has a #raw("ROW") type column with several fields. If a query only accesses one field, dereference pushdown allows the file reader to read only that single field within the row. The same applies to fields of a row nested within the top level row. This can result in significant savings in the amount of data read from the storage system.

#anchor("ref-aggregation-pushdown")
...
You can check if pushdown for a specific query is performed by looking at the #link(label("doc-sql-explain"))[EXPLAIN plan] of the query. If an aggregate function is successfully pushed down to the connector, the explain plan does #strong[not] show that #raw("Aggregate") operator. The explain plan only shows the operations that are performed by Trino.

As an example, we loaded the TPC-H data set into a PostgreSQL database and then queried it using the PostgreSQL connector:

#code-block(none, "SELECT regionkey, count(*)
FROM nation
GROUP BY regionkey;")

You can get the explain plan by prepending the above query with #raw("EXPLAIN"):
...
The explain plan for this query does not show any #raw("Aggregate") operator with the #raw("count") function, as this operation is now performed by the connector. You can see the #raw("count(*)") function as part of the PostgreSQL #raw("TableScan") operator. This shows you that the pushdown was successful.

#code-block("text", "Fragment 0 [SINGLE]
...
As a result, the explain plan shows the #raw("Aggregate") operation being performed by Trino. This is a clear sign that now pushdown to the remote data source is not performed, and instead Trino performs the aggregate processing.
...
- complex grouping operations such as #raw("ROLLUP"), #raw("CUBE"), or #raw("GROUPING SETS")
- expressions inside the aggregation function call: #raw("sum(a * b)")
- coercions: #raw("sum(integer_column)")
- #link(label("ref-aggregate-function-ordering-during-aggregation"))[aggregations with ordering]
- #link(label("ref-aggregate-function-filtering-during-aggregation"))[aggregations with filter]

#anchor("ref-join-pushdown")
...
You can verify if pushdown for a specific join is performed by looking at the #link(label("doc-sql-explain"))[EXPLAIN]  plan of the query. The explain plan does not show a #raw("Join") operator, if the join is pushed down to the data source by the connector:

#code-block(none, "EXPLAIN SELECT c.custkey, o.orderkey
FROM orders o JOIN customer c ON c.custkey = o.custkey;")

The following plan results from the PostgreSQL connector querying TPC-H data in a PostgreSQL database. It does not show any #raw("Join") operator as a result of the successful join push down.
...
#anchor("ref-limit-pushdown")

== Limit pushdown

A #link(label("ref-limit-clause"))[limit-clause] reduces the number of returned records for a statement. Limit pushdown enables a connector to push processing of such queries of unsorted record to the underlying data source.
...
Queries include sections such as #raw("LIMIT N") or #raw("FETCH FIRST N ROWS").

Implementation and support is connector-specific since different data sources have varying capabilities.

#anchor("ref-topn-pushdown")

== Top-N pushdown

The combination of a #link(label("ref-limit-clause"))[limit-clause] with an #link(label("ref-order-by-clause"))[order-by-clause] creates a small set of records to return out of a large sorted dataset. It relies on the order to determine which records need to be returned, and is therefore quite different to optimize compared to a #link(label("ref-limit-pushdown"))[limit-pushdown].
...
Queries include sections such as #raw("ORDER BY ... LIMIT N") or #raw("ORDER BY ... FETCH FIRST N ROWS").
...
#code-block(none, "SELECT id, name
...
#code-block(none, "EXPLAIN SELECT id, name
FROM postgresql.public.company
ORDER BY id
LIMIT 5;")
...
Second, an example of a Top-N query on the #raw("tpch") connector which does not support Top-N pushdown functionality:

#code-block(none, "SELECT custkey, name
...
In the preceding query plan, the Top-N operation #raw("TopN[5 by (custkey ASC NULLS LAST)]") is being applied in the #raw("Fragment 0") by Trino and not by the source database.

Note that, compared to the query executed on top of the #raw("tpch") connector, the explain plan of the query applied on top of the #raw("postgresql") connector is missing the reference to the operation #raw("TopN[5 by (id ASC NULLS LAST)]") in the #raw("Fragment 0"). The absence of the #raw("TopN") Trino operator in the #raw("Fragment 0") from the query plan demonstrates that the query benefits of the Top-N pushdown optimization.
