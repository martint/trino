...
Properties related to spilling are described in properties-spilling.

#warning[
The spill to disk feature and implementation are a legacy functionality of Trino. Consider using #link(label("doc-admin-fault-tolerant-execution"))[Fault-tolerant execution] with the #raw("task") retry policy and a configured #link(label("ref-fte-exchange-manager"))[Fault-tolerant execution].
]

== Memory management and spill

By default, Trino kills queries, if the memory requested by the query execution exceeds session properties #raw("query_max_memory") or #raw("query_max_memory_per_node"). This mechanism ensures fairness in allocation of memory to queries, and prevents deadlock caused by memory allocation. It is efficient when there is a lot of small queries in the cluster, but leads to killing large queries that don't stay within the limits.
...
Please note that enabling spill-to-disk does not guarantee execution of all memory intensive queries. It is still possible that the query runner fails to divide intermediate data into chunks small enough so that every chunk fits into memory, leading to #raw("Out of memory") errors while loading the data from disk.

== Spill disk space

Spilling intermediate results to disk, and retrieving them back, is expensive in terms of IO operations. Thus, queries that use spill likely become throttled by disk. To increase query performance, it is recommended to provide multiple paths on separate local devices for spill \(property #raw("spiller-spill-path") in properties-spilling\).
...
Trino treats spill paths as independent disks \(see #link("https://wikipedia.org/wiki/Non-RAID_drive_architectures#JBOD")[JBOD]\), so there is no need to use RAID for spill.

== Spill compression

When spill compression is enabled with the #link(label("doc-admin-properties-spilling"))[#raw("spill-compression-codec") property], spilled pages are compressed, before being written to disk. Enabling this feature can reduce disk IO at the cost of extra CPU load to compress and decompress spilled pages.

== Spill encryption

When spill encryption is enabled \(#raw("spill-encryption-enabled") property in properties-spilling\), spill contents are encrypted with a randomly generated \(per spill file\) secret key. Enabling this increases CPU load and reduces throughput of spilling to disk, but can protect spilled data from being recovered from spill files. Consider reducing the value of #raw("memory-revoking-threshold") when spill encryption is enabled, to account for the increase in latency of spilling.
...
When the task concurrency is greater than one, the build table is partitioned. The number of partitions is equal to the value of the #raw("task.concurrency") configuration parameter \(see properties-task\).
...
With this mechanism, the peak memory used by the join operator can be decreased to the size of the largest build table partition. Assuming no data skew, this is #raw("1 / task.concurrency") times the size of the whole build table.
...
If your trying to sort a larger amount of data, a significant amount of memory may be needed. When spill to disk for #raw("order by") is enabled, if there is not enough memory, intermediate sorted results are written to disk. They are loaded back and merged with a lower memory footprint.
