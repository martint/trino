...
#anchor("doc-connector-thrift")
= Thrift connector

The Thrift connector makes it possible to integrate with external storage systems without a custom Trino connector implementation by using #link("https://thrift.apache.org/")[Apache Thrift] on these servers. It is therefore generic and can provide access to any backend, as long as it exposes the expected API by using Thrift.

In order to use the Thrift connector with an external system, you need to implement the #raw("TrinoThriftService") interface, found below. Next, you configure the Thrift connector to point to a set of machines, called Thrift servers, that implement the interface. As part of the interface implementation, the Thrift servers provide metadata, splits and data. The connector randomly chooses a server to talk to from the available instances for metadata calls, or for data calls unless the splits include a list of addresses. All requests are assumed to be idempotent and can be retried freely among any server.
...
- A #link(label("ref-trino-thrift-service"))[trino-thrift-service] for your system.

== Configuration

To configure the Thrift connector, create a catalog properties file #raw("etc/catalog/example.properties") with the following content, replacing the properties as appropriate:

#code-block("text", "connector.name=trino_thrift
trino.thrift.client.addresses=host:port,host:port")

=== Multiple Thrift systems

You can have as many catalogs as you need, so if you have additional Thrift systems to connect to, simply add another properties file to #raw("etc/catalog") with a different name, making sure it ends in #raw(".properties").
...
#list-table((
  ...
  ([#raw("trino.thrift.client.addresses")], [Location of Thrift servers],),
  ([#raw("trino-thrift.max-response-size")], [Maximum size of data returned from Thrift server],),
  ([#raw("trino-thrift.metadata-refresh-threads")], [Number of refresh threads for metadata cache],),
  ([#raw("trino.thrift.client.max-retries")], [Maximum number of retries for failed Thrift requests],),
  ([#raw("trino.thrift.client.max-backoff-delay")], [Maximum interval between retry attempts],),
  ([#raw("trino.thrift.client.min-backoff-delay")], [Minimum interval between retry attempts],),
  ([#raw("trino.thrift.client.max-retry-time")], [Maximum duration across all attempts of a Thrift request],),
  ([#raw("trino.thrift.client.backoff-scale-factor")], [Scale factor for exponential back off],),
  ([#raw("trino.thrift.client.connect-timeout")], [Connect timeout],),
  ([#raw("trino.thrift.client.request-timeout")], [Request timeout],),
  ([#raw("trino.thrift.client.socks-proxy")], [SOCKS proxy address],),
  ([#raw("trino.thrift.client.max-frame-size")], [Maximum size of a raw Thrift response],),
  ([#raw("trino.thrift.client.transport")], [Thrift transport type \(#raw("UNFRAMED"), #raw("FRAMED"), #raw("HEADER")\)],),
  ([#raw("trino.thrift.client.protocol")], [Thrift protocol type \(#raw("BINARY"), #raw("COMPACT"), #raw("FB_COMPACT")\)],)
), header-rows: 1)

=== #raw("trino.thrift.client.addresses")

Comma-separated list of thrift servers in the form of #raw("host:port"). For example:

#code-block("text", "trino.thrift.client.addresses=192.0.2.3:7777,192.0.2.4:7779")

This property is required; there is no default.

=== #raw("trino-thrift.max-response-size")
...
This property is optional; the default is #raw("16MB").

=== #raw("trino-thrift.metadata-refresh-threads")

Number of refresh threads for metadata cache.

This property is optional; the default is #raw("1").

#anchor("ref-trino-thrift-service")

== TrinoThriftService implementation

The following IDL describes the #raw("TrinoThriftService") that must be implemented:

#code-block("thrift", "enum TrinoThriftBound {
  BELOW = 1;
  EXACTLY = 2;
  ABOVE = 3;
}
...
#anchor("ref-thrift-type-mapping")

== Type mapping

The Thrift service defines data type support and mappings to Trino data types.

#anchor("ref-thrift-sql-support")

== SQL support

The connector provides #link(label("ref-sql-globally-available"))[globally available] and #link(label("ref-sql-read-operations"))[read operation] statements to access data and metadata in your Thrift service.
