...
#anchor("doc-connector-kafka-tutorial")
= Kafka connector tutorial

== Introduction

The kafka for Trino allows access to live topic data from Apache Kafka using Trino. This tutorial shows how to set up topics, and how to create the topic description files that back Trino tables.

== Installation

This tutorial assumes familiarity with Trino and a working local Trino installation \(see #link(label("doc-installation-deployment"))[Deploying Trino]\). It focuses on setting up Apache Kafka and integrating it with Trino.

=== Step 1: Install Apache Kafka

Download and extract #link("https://kafka.apache.org/")[Apache Kafka].
...
#code-block("text", "$ bin/zookeeper-server-start.sh config/zookeeper.properties
[2013-04-22 15:01:37,495] INFO Reading configuration from: config/zookeeper.properties (org.apache.zookeeper.server.quorum.QuorumPeerConfig)
...")

#code-block("text", "$ bin/kafka-server-start.sh config/server.properties
[2013-04-22 15:01:47,028] INFO Verifying properties (kafka.utils.VerifiableProperties)
[2013-04-22 15:01:47,051] INFO Property socket.send.buffer.bytes is overridden to 1048576 (kafka.utils.VerifiableProperties)
...")

This starts Zookeeper on port #raw("2181") and Kafka on port #raw("9092").

=== Step 2: Load data

Download the tpch-kafka loader from Maven Central:

#code-block("text", "$ curl -o kafka-tpch https://repo1.maven.org/maven2/de/softwareforge/kafka_tpch_0811/1.0/kafka_tpch_0811-1.0.sh
$ chmod 755 kafka-tpch")

Now run the #raw("kafka-tpch") program to preload a number of topics with tpch data:

#code-block("text", "$ ./kafka-tpch load --brokers localhost:9092 --prefix tpch. --tpch-type tiny
...
In your Trino installation, add a catalog properties file #raw("etc/catalog/kafka.properties") for the Kafka connector. This file lists the Kafka nodes and topics:
...
Because the Kafka tables all have the #raw("tpch.") prefix in the configuration, the tables are in the #raw("tpch") schema. The connector is mounted into the #raw("kafka") catalog, because the properties file is named #raw("kafka.properties").

Start the #link(label("doc-client-cli"))[Trino CLI]:

#code-block("text", "$ ./trino --catalog kafka --schema tpch")

List the tables to verify that things are working:

#code-block("text", "trino:tpch> SHOW TABLES;
  Table
----------
 customer
 lineitem
...
#code-block("text", "trino:tpch> DESCRIBE customer;
...
The data from Kafka can be queried using Trino, but it is not yet in actual table shape. The raw data is available through the #raw("_message") and #raw("_key") columns, but it is not decoded into columns. As the sample data is in JSON format, the #link(label("doc-functions-json"))[JSON functions and operators] built into Trino can be used to slice the data.

=== Step 5: Add a topic description file

The Kafka connector supports topic description files to turn raw data into table format. These files are located in the #raw("etc/kafka") folder in the Trino installation and must end with #raw(".json"). It is recommended that the file name matches the table name, but this is not necessary.

Add the following file as #raw("etc/kafka/tpch.customer.json") and restart Trino:
...
The customer table now has an additional column: #raw("kafka_key").
...
The topic definition file maps the internal Kafka key, which is a raw long in eight bytes, onto a Trino #raw("BIGINT") column.

=== Step 6: Map all the values from the topic message onto columns

Update the #raw("etc/kafka/tpch.customer.json") file to add fields for the message, and restart Trino. As the fields in the message are JSON, it uses the #raw("JSON") data format. This is an example, where different data formats are used for the key and the message.
...
Now for all the fields in the JSON of the message, columns are defined and the sum query from earlier can operate on the #raw("account_balance") column directly:
...
Now all the fields from the #raw("customer") topic messages are available as Trino table columns.
...
#code-block("text", "$ curl -o twistr https://repo1.maven.org/maven2/de/softwareforge/twistr_kafka_0811/1.2/twistr_kafka_0811-1.2.sh
$ chmod 755 twistr")

- Create a developer account at #link("https://dev.twitter.com/")[https:\/\/dev.twitter.com\/] and set up an access and consumer token.
- Create a #raw("twistr.properties") file and put the access and consumer key and secrets into it:

#code-block("text", "twistr.access-token-key=...
...
Add the tweets table to the #raw("etc/catalog/kafka.properties") file:
...
Add a topic definition file for the Twitter feed as #raw("etc/kafka/tweets.json"):
...
As this table does not have an explicit schema name, it is placed into the #raw("default") schema.

==== Feed live data

Start the twistr tool:

#code-block("text", "$ java -Dness.config.location=file:$(pwd) -Dness.config=twistr -jar ./twistr")

#raw("twistr") connects to the Twitter API and feeds the "sample tweet" feed into a Kafka topic called #raw("twitter_feed").

Now run queries against live data:

#code-block("text", "$ ./trino --catalog kafka --schema default
...
The tweets feed, that was set up in the last step, contains a timestamp in RFC 2822 format as #raw("created_at") attribute in each tweet.

#code-block("text", "trino:default> SELECT DISTINCT json_extract_scalar(_message, '$.created_at')) AS raw_date
             -> FROM tweets LIMIT 5;
            raw_date
--------------------------------
...
The topic definition file for the tweets table contains a mapping onto a timestamp using the #raw("rfc2822") converter:
...
#code-block("text", "trino:default> SELECT created_at, raw_date FROM (
