...
- All tests must use statically imported AssertJ assertions, typically from #raw("org.assertj.core.api.Assertions").
- Test class names must start with #raw("Test"), for example #raw("TestExample")
- Test classes should be defined as package-private and final.
- Test method must start with #raw("test"), for example #raw("testExplain()")
...
JUnit and the JVM take care of test life cycle and memory management. Avoid manual steps such as nulling out fields in #raw("@After") methods to “free memory”. It is safe to assign memory intensive objects to final fields, as the class is automatically dereferenced after the test run.

=== Use simple resource initialization

Prefer resource initialization in constructors and tear them down in #raw("@After") methods if necessary. This approach, combined with not nulling fields, allows the fields to be final and behave like any #raw("Closeable") class in normal Java code Consider using the Guava #raw("Closer") class to simplify cleanup.

=== Keep test setup and teardown simple

Avoid the #raw("@Before")\/#raw("@After") each test method style of setup and teardown.
...
Existing flaky tests using the legacy TestNG library can be marked with the #raw("@Flaky") annotation temporarily to improve CI reliability until a fix is implemented:
...
New tests with the #raw("@Flaky") annotation can not be introduced, since new tests must use JUnit. Rewrite the test to be stable or avoid the test altogether.
...
=== Avoid using #raw("Assumptions.abort()")

The approach to use #raw("Assumptions.abort()") to skip a test, especially deep in the call stack, makes it difficult to debug tests failures. The #raw("abort()") works by throwing an exception, which can be caught by intervening code inadvertently, leading to misleading stack traces and test failures.
...
- Start the name with #raw("assert"), for example #raw("assertSomeLogicWorks")
...
Use #raw("PER_CLASS") for instances because #raw("QueryAssertions") is too expensive to create per-method, and a allow parallel execution of tests with #raw("CONCURRENT"):

#code-block("java", "@TestInstance(PER_CLASS)
...
Avoid managing the lifecycle of a Closeable like a connection with #raw("@BeforeEach")\/#raw("@AfterEach") to reduce overhead:

#code-block("java", "@TestInstance(PER_METHOD)
final class Test
{
...
#code-block("java", "final class Test
{
...
#code-block("java", "@DataProvider(name = \"data\")
...
#code-block("java", "@Test(dataProvider = \"parallelTests\")
