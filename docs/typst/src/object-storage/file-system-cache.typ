...
#anchor("doc-object-storage-file-system-cache")
= File system cache
...
Trino includes support for caching these files with the help of the open source #link("https://github.com/Alluxio/alluxio")[Alluxio] libraries with catalogs using the following connectors:
...
#strong[Increased query performance]
...
#anchor("ref-fs-cache-configuration")
...
#list-table((
  ...
  ([#raw("fs.cache.enabled")], [Enable object storage caching. Defaults to no caching with the value #raw("false").],),
  ([#raw("fs.cache.directories")], [Required, comma-separated list of absolute paths to directories to use for caching. All directories must exist on the coordinator and all workers. Trino must have read and write permissions for files and nested directories. A valid example with only one directory is #raw("/tmp/trino-cache").

Directories must be specific for each catalog with caching enabled. When enabling caching in multiple catalogs, you must use different directories and set the values for #raw("fs.cache.max-sizes") or #raw("fs.cache.max-disk-usage-percentages") accordingly.],),
  ([#raw("fs.cache.max-sizes")], [Comma-separated list of maximum #link(label("ref-prop-type-data-size"))[data sizes] for each caching directory. Order of values must be identical to the directories list. Configuring either #raw("fs.cache.max-sizes") or #raw("fs.cache.max-disk-usage-percentages") is required.],),
  ([#raw("fs.cache.max-disk-usage-percentages")], [Comma-separated list of maximum percentage values of the used disk for each directory. Each value is an integer between 1 and 100. Order of values must be identical to the directories list. If multiple directories use the same disk, ensure that total percentages per drive remains below 100 percent. Configuring either #raw("fs.cache.max-sizes") or #raw("fs.cache.max-disk-usage-percentages") is required.],),
  ([#raw("fs.cache.ttl")], [The maximum #link(label("ref-prop-type-duration"))[duration] for objects to remain in the cache before eviction. Defaults to #raw("7d"). The minimum value of #raw("0s") means that caching is effectively turned off.],),
  ([#raw("fs.cache.preferred-hosts-count")], [The number of preferred nodes for caching files. Defaults to 2. Processing identifies and subsequently prefers using specific nodes. If the preferred nodes identified for caching a split are unavailable or too busy, then an available node is chosen at random from the cluster. More information in #link(label("ref-fs-cache-distributed"))[File system cache].],),
  ([#raw("fs.cache.page-size")], [The page #link(label("ref-prop-type-data-size"))[data size] used for caching data. Each transfer of files uses at least this amount of data. Defaults to #raw("1MB"). Values must be between #raw("64kB") and #raw("15MB"). Larger value potentially result in too much data transfer smaller values are less efficient since they result in more individual downloads.],)
), header-rows: 1, title: "File system cache configuration properties")

== Monitoring

The cache exposes the #link("https://docs.alluxio.io/ee-da/user/stable/en/reference/Metrics-List.html#client-metrics")[Alluxio JMX client metrics] under the #raw("org.alluxio") package, and metrics on external reads and cache reads under #raw("io.trino.filesystem.alluxio.AlluxioCacheStats").

The cache code uses #link(label("doc-admin-opentelemetry"))[OpenTelemetry tracing].
