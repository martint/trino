...
#note[
The book #link("https://trino.io/trino-the-definitive-guide.html")[Trino: The Definitive Guide] and the research paper #link("https://trino.io/paper.html")[Presto: SQL on Everything] can provide further information about Trino and the concepts in use.
]

#anchor("ref-trino-concept-architecture")

== Architecture

Trino is a distributed query engine that processes data in parallel across multiple servers. There are two types of Trino servers, #link(label("ref-trino-concept-coordinator"))[coordinators] and #link(label("ref-trino-concept-worker"))[workers]. The following sections describe these servers and other components of Trino's architecture.

#anchor("ref-trino-concept-cluster")

=== Cluster

A Trino cluster consists of several Trino #link(label("ref-trino-concept-node"))[nodes] - one #link(label("ref-trino-concept-coordinator"))[coordinator] and zero or more #link(label("ref-trino-concept-worker"))[workers]. Users connect to the coordinator with their SQL query tool. The coordinator collaborates with the workers. The coordinator and the workers access the connected #link(label("ref-trino-concept-data-source"))[data sources]. This access is configured in #link(label("ref-trino-concept-catalog"))[catalogs].
...
#anchor("ref-trino-concept-node")

=== Node

Any Trino server in a specific Trino cluster is considered a #strong[node] of the #link(label("ref-trino-concept-cluster"))[cluster]. Technically this refers to the Java process running the Trino program, but node is often used to refer to the computer running the process due to the recommendation to run only one Trino process per computer.

#anchor("ref-trino-concept-coordinator")
...
#anchor("ref-trino-concept-worker")
...
#anchor("ref-trino-concept-client")

== Client

Clients allow you to connect to Trino, submit SQL queries, and receive the results. Clients can access all configured data sources using #link(label("ref-trino-concept-catalog"))[catalogs]. Clients are full-featured client applications or client drivers and libraries that allow you to connect with any application supporting that driver, or even your own custom application or script.

Clients applications include command line tools, desktop applications, web-based applications, and software-as-a-service solutions with features such as interactive SQL query authoring with editors, or rich user interfaces for graphical query creation, query running and result rendering, visualizations with charts and graphs, reporting, and dashboard creation.

Client application that support other query languages or user interface components to build a query, must translate each request to #link(label("doc-language"))[SQL as supported by Trino].

More details are available in the #link(label("doc-client"))[Trino client documentation].

#anchor("ref-trino-concept-plugin")

== Plugin

Trino uses a plugin architecture to extend its capabilities and integrate with various data sources and other systems. Details about different types of plugins, installation, removal, and other aspects are available in the #link(label("doc-installation-plugins"))[Plugin documentation].

#anchor("ref-trino-concept-data-source")
...
#link("https://trino.io/ecosystem/data-source")[A comprehensive list with more details for each data source is available on the Trino website].

Data sources provide the data for Trino to query. Configure a #link(label("ref-trino-concept-catalog"))[catalog] with the required Trino #link(label("ref-trino-concept-connector"))[connector] for the specific data source to access the data. With Trino you are ready to use any supported #link(label("ref-trino-concept-client"))[client] to query the data sources using SQL and the features of your client.
...
#anchor("ref-trino-concept-connector")

=== Connector

A connector adapts Trino to a data source such as a data lake using Hadoop\/Hive or Apache Iceberg, or a relational database such as PostgreSQL. You can think of a connector the same way you think of a driver for a database. It is an implementation of Trino's #link(label("doc-develop-spi-overview"))[service provider interface \(SPI\)], which allows Trino to interact with a resource using a standard API.

Trino contains #link(label("doc-connector"))[many built-in connectors]:

- Connectors for data lakes and lakehouses including the #link(label("doc-connector-delta-lake"))[Delta Lake], #link(label("doc-connector-hive"))[Hive], #link(label("doc-connector-hudi"))[Hudi], and #link(label("doc-connector-iceberg"))[Iceberg] connectors.
- Connectors for relational database management systems, including the #link(label("doc-connector-mysql"))[MySQL], #link(label("doc-connector-postgresql"))[PostgreSQL], #link(label("doc-connector-oracle"))[Oracle], and #link(label("doc-connector-sqlserver"))[SQL Server] connectors.
- Connectors for a variety of other systems, including the #link(label("doc-connector-cassandra"))[Cassandra], #link(label("doc-connector-clickhouse"))[ClickHouse], #link(label("doc-connector-opensearch"))[OpenSearch], #link(label("doc-connector-pinot"))[Pinot], #link(label("doc-connector-prometheus"))[Prometheus], #link(label("doc-connector-singlestore"))[SingleStore], and #link(label("doc-connector-snowflake"))[Snowflake] connectors.
- A number of other utility connectors such as the #link(label("doc-connector-jmx"))[JMX], #link(label("doc-connector-system"))[System], and #link(label("doc-connector-tpch"))[TPC-H] connectors.

Every catalog uses a specific connector. If you examine a catalog configuration file, you see that each contains a mandatory property #raw("connector.name") with the value identifying the connector.

#anchor("ref-trino-concept-catalog")

=== Catalog

A Trino catalog is a collection of configuration properties used to access a specific data source, including the required connector and any other details such as credentials and URL. Catalogs are defined in properties files stored in the Trino configuration directory. The name of the properties file determines the name of the catalog. For example, the properties file #raw("etc/example.properties") results in a catalog name #raw("example").

You can configure and use many catalogs, with different or identical connectors, to access different data sources. For example, if you have two data lakes, you can configure two catalogs in a single Trino cluster that both use the Hive connector, allowing you to query data from both clusters, even within the same SQL query. You can also use a Hive connector for one catalog to access a data lake, and use the Iceberg connector for another catalog to access the data lakehouse. Or, you can configure different catalogs to access different PostgreSQL database. The combination of different catalogs is determined by your needs to access different data sources only.

A catalog contains one or more schemas, which in turn contain objects such as tables, views, or materialized views. When addressing an objects such as tables in Trino, the fully-qualified name is always rooted in a catalog. For example, a fully-qualified table name of #raw("example.test_data.test") refers to the #raw("test") table in the #raw("test_data") schema in the #raw("example") catalog.
...
A table is a set of unordered rows, which are organized into named columns with #link(label("doc-language-types"))[types]. This is the same as in any relational database. Type mapping from source data to Trino is defined by the connector, varies across connectors, and is documented in the specific connector documentation, for example the #link(label("ref-postgresql-type-mapping"))[type mapping in the PostgreSQL connector].
...
#anchor("ref-trino-concept-stage")
...
#anchor("ref-trino-concept-splits")
