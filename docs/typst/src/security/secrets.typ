...
The secrets support in Trino allows you to use environment variables as values for any configuration property. All properties files used by Trino, including #raw("config.properties") and catalog properties files, are supported. When loading the properties, Trino replaces the reference to the environment variable with the value of the environment variable.
...
#code-block("text", "export DB_PASSWORD=my-super-secret-pwd")

To use this variable in the properties file, you reference it with the syntax #raw("${ENV:VARIABLE}"). For example, if you want to use the password in a catalog properties file like #raw("etc/catalog/db.properties"), add the following line:

#code-block("properties", "connection-password=${ENV:DB_PASSWORD}")
