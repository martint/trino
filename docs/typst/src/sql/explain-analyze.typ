...
#anchor("doc-sql-explain-analyze")
= EXPLAIN ANALYZE

== Synopsis

#code-block("text", "EXPLAIN ANALYZE [VERBOSE] statement")

== Description

Execute the statement and show the distributed execution plan of the statement along with the cost of each operation.

The #raw("VERBOSE") option will give more detailed information and low-level statistics; understanding these may require knowledge of Trino internals and implementation details.
...
#code-block("sql", "EXPLAIN ANALYZE SELECT count(*), clerk FROM orders
...
When the #raw("VERBOSE") option is used, some operators may report additional information. For example, the window function operator will output the following:

#code-block(none, "EXPLAIN ANALYZE VERBOSE SELECT count(clerk) OVER() FROM orders
