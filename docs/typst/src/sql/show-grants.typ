...
#anchor("doc-sql-show-grants")
= SHOW GRANTS

== Synopsis

#code-block("text", "SHOW GRANTS [ ON [ TABLE ] table_name ]")
...
List the grants for the current user on table #raw("orders"):

#code-block(none, "SHOW GRANTS ON TABLE orders;")

List the grants for the current user on all the tables in all schemas of the current catalog:

#code-block(none, "SHOW GRANTS;")

== Limitations

Some connectors have no support for #raw("SHOW GRANTS"). See connector documentation for more details.
