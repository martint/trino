...
#anchor("doc-sql-explain")
= EXPLAIN

== Synopsis

#code-block("text", "EXPLAIN [ ( option [, ...] ) ] statement")

where #raw("option") can be one of:

#code-block("text", "FORMAT { TEXT | GRAPHVIZ | JSON }
...
/ #raw("SINGLE"): Fragment is executed on a single node.
/ #raw("HASH"): Fragment is executed on a fixed number of nodes with the input data distributed using a hash function.
/ #raw("ROUND_ROBIN"): Fragment is executed on a fixed number of nodes with the input data distributed in a round-robin fashion.
/ #raw("BROADCAST"): Fragment is executed on a fixed number of nodes with the input data broadcasted to all nodes.
/ #raw("SOURCE"): Fragment is executed on nodes where input splits are accessed.
...
#warning[
EXPLAIN \(TYPE LOGICAL\) is deprecated and will be removed in a future release. Use EXPLAIN \(TYPE DISTRIBUTED\) instead.
]

Process the supplied query statement and create a logical plan in text format:

#code-block(none, "EXPLAIN (TYPE LOGICAL) SELECT regionkey, count(*) FROM nation GROUP BY 1;")
...
#code-block(none, "EXPLAIN (TYPE LOGICAL, FORMAT JSON) SELECT regionkey, count(*) FROM nation GROUP BY 1;")
...
#code-block(none, "EXPLAIN (TYPE DISTRIBUTED) SELECT regionkey, count(*) FROM nation GROUP BY 1;")
...
#code-block(none, "EXPLAIN (TYPE DISTRIBUTED, FORMAT JSON) SELECT regionkey, count(*) FROM nation GROUP BY 1;")
...
#code-block(none, "EXPLAIN (TYPE VALIDATE) SELECT regionkey, count(*) FROM nation GROUP BY 1;")

#code-block("text", " Valid
-------
 true")
...
#code-block(none, "EXPLAIN (TYPE VALIDATE) SELET 1=0;")

#code-block("text", "Query 20220929_234840_00001_vjwxj failed: line 1:25: mismatched input 'SELET'.
...
Similarly if semantic issues are detected, such as an invalid object name #raw("nations") instead of #raw("nation"), the error message returns useful information:

#code-block(none, "EXPLAIN(TYPE VALIDATE) SELECT * FROM tpch.tiny.nations;")

#code-block("text", "Query 20220929_235059_00003_vjwxj failed: line 1:15: Table 'tpch.tiny.nations' does not exist
...
#code-block(none, "EXPLAIN (TYPE IO, FORMAT JSON) INSERT INTO test_lineitem
