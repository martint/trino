        ...
        implements ColumnReader
{
    ...
    public static final int[] EMPTY_REPETITION_LEVELS = new int[0];
    ...
    protected ValueDecoder<BufferType> valueDecoder;
     ...
     */
    private BufferType createTemporaryBuffer(int size)
    {
    ...
    private void unpackNullValues(BufferType source, BufferType destination, boolean[] isNull, int destOffset, int nonNullCount)
    {
        int srcOffset = 0;
        ...
        while (remainingInBatch > 0) {
            ...
            if (nonNullCount > 0) {
                if (nonNullCount < chunkSize) {
                    ...
                    BufferType tmpBuffer = createTemporaryBuffer(nonNullCount);
                    valueDecoder.read(tmpBuffer, 0, nonNullCount);
                    unpackNullValues(tmpBuffer, values, isNull, offset, nonNullCount);
                }
                else {
    ...
    private void readFlatPageV1(DataPageV1 page)
    {
        ...
        if (definitionEncoding == RLE) {
            ...
            // See org.apache.parquet.column.Encoding.getValuesReader for RLE for reference.
            if (field.getDescriptor().getMaxDefinitionLevel() == 0) {
                ...
                definitionLevelDecoder = new FlatDefinitionLevelDecoder(buffer.slice(Integer.BYTES, bufferSize));
    ...
    private void readFlatPageV2(DataPageV2 page)
    {
        ...
        definitionLevelDecoder = new FlatDefinitionLevelDecoder(page.getDefinitionLevels());
