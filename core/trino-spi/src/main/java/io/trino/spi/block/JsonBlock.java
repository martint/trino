        ...
        implements ValueBlock
{
    ...
    // hasNull is expensive to recompute (O(positionCount) scan) and is called in hot loops;
    // cache the first-computed value here. -1 = unknown, 0 = false, 1 = true.
    private static final byte HAS_NULL_UNKNOWN = -1;
    private static final byte HAS_NULL_FALSE = 0;
    private static final byte HAS_NULL_TRUE = 1;
    private volatile byte cachedHasNull = HAS_NULL_UNKNOWN;
    ...
    public static JsonBlock create(int positionCount, Block parsedItem, Optional<boolean[]> isNullOptional)
    {
        if (isNullOptional.isPresent()) {
            checkArrayRange(isNullOptional.get(), 0, positionCount);
        }
    ...
    private JsonBlock(int startOffset, int positionCount, @Nullable boolean[] isNull, Block parsedItem)
    {
        ...
        if (isNull != null) {
            for (int position = 0; position < positionCount; position++) {
                if (isNull[startOffset + position] && !parsedItem.isNull(startOffset + position)) {
                    throw new IllegalArgumentException("Parsed-item for null JSON must be null: position %d".formatted(position));
                }
            }
        }
    ...
    public boolean hasNull()
    {
        ...
        if (cachedHasNull == HAS_NULL_UNKNOWN) {
            boolean found = false;
            ...
            cachedHasNull = found ? HAS_NULL_TRUE : HAS_NULL_FALSE;
        }
        return cachedHasNull == HAS_NULL_TRUE;
    }
