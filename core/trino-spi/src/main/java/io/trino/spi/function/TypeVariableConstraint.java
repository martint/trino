...
public class TypeVariableConstraint
{
    ...
    public String toString()
    {
        ...
        if (!castableTo.isEmpty()) {
            value += castableTo.stream().map(TypeTemplates::render).collect(joining(", ", ":castableTo(", ")"));
        }
        if (!castableFrom.isEmpty()) {
            value += castableFrom.stream().map(TypeTemplates::render).collect(joining(", ", ":castableFrom(", ")"));
        }
        return value;
    }
    ...
    public static class TypeVariableConstraintBuilder
    {
        ...
        {
            return castableTo(TypeTemplates.fromTypeDescriptor(type.getTypeSignature()));
        }
        ...
        {
            return castableFrom(TypeTemplates.fromTypeDescriptor(type.getTypeSignature()));
        }
