        ...
        implements BlockBuilder
{
    ...
    public void append(ValueBlock block, int position)
    {
        ...
        if (block instanceof VariableWidthBlock variableWidthBlock) {
            // JSON columns from connector readers (Parquet/ORC/JDBC) arrive as raw text in
            // VariableWidthBlock; copy the text payload into the parsed-item builder.
            parsedItemBlockBuilder.writeEntry(variableWidthBlock.getSlice(position));
            entryAdded(false);
            return;
        }
    ...
    public void appendRange(ValueBlock block, int offset, int length)
    {
        ...
        if (block instanceof VariableWidthBlock variableWidthBlock) {
            for (int i = 0; i < length; i++) {
                append(variableWidthBlock, offset + i);
            }
            return;
        }
    ...
    public void appendPositions(ValueBlock block, int[] positions, int offset, int length)
    {
        ...
        if (block instanceof VariableWidthBlock variableWidthBlock) {
            for (int i = 0; i < length; i++) {
                append(variableWidthBlock, positions[offset + i]);
            }
            return;
        }
