...
public final class JsonItemSemantics
{
    ...
    public static long hash(JsonItem item)
    {
        ...
        return switch (item) {
            ...
            case JsonArray array -> {
                ...
                yield mix(hash, array.elements().size());
            }
            case JsonObject object -> {
                long sum = 0;
                long xor = 0;
                for (JsonObjectMember member : object.members()) {
                    long hash = mix(saltedStringHash(member.key()), hash(member.value()));
                    sum += hash;
                    xor ^= Long.rotateLeft(hash, 17);
                }
                yield mix(mix(HASH_OBJECT, object.members().size()), mix(sum, xor));
            }
    ...
    private static NumericKind numericKind(TypedValue value)
    {
    ...
    private static NumericKind classifyDouble(double number)
    {
    ...
    private static long nonFiniteHash(TypedValue value)
    {
        return switch (numericKind(value)) {
