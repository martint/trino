 ...
 *
 * <p>Equality and hashing across the {@link JsonPathItem} hierarchy follow the default
 ...
 * non-finite-by-kind), use {@link JsonItemSemantics#equals(JsonPathItem, JsonPathItem)} and
 * {@link JsonItemSemantics#hash(JsonPathItem)}. Java equality is intentionally not overridden
 * to keep collection bucketing predictable; callers must not rely on Java equality to
 * deduplicate across representations (e.g. mixing an {@link EncodedJsonItem} with the same
 * value as a materialized {@link JsonArrayItem} in a {@code HashSet}).
 */
