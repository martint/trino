...
final class SchedulingQueue<G, T>
{
    ...
    public void enqueue(G group, T task, long deltaWeight)
    {
        ...
        if (previousState == BLOCKED) {
            ...
            info.adjustWeight(baselineWeight() - blockedGroup.savedBaseline());
        }
    ...
    private void transitionToBlocked(G group, Group<T> info)
    {
        ...
        blocked.put(group, new Blocked(baselineWeight()));
        baselineWeights.removeIfPresent(group);
        runnableQueue.removeIfPresent(group);
    }
    ...
    {
        return new StringJoiner(", ", SchedulingQueue.class.getSimpleName() + "[", "]")
                .add("runnableQueue=" + runnableQueue)
                .add("groups=" + groups)
                .add("baselineWeights=" + baselineWeights)
                .add("blocked=" + blocked)
                .toString();
    }
    ...
    private static class Group<T>
    {
        ...
        public void enqueue(T task, long deltaWeight)
        {
            ...
            if (info == null) {
                ...
                info.adjustWeight(baselineWeight() - blockedTask.savedBaseline());
            }
        ...
        public void block(T task, long deltaWeight)
        {
            ...
            blocked.put(task, new Blocked(baselineWeight()));
    ...
    private record Blocked(long savedBaseline)
    {
    }
}
