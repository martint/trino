...
/**
 * Lowers a correlated {@code IN} {@link ApplyNode} into a {@link CorrelatedJoinNode} that the
 * unified dependent-join framework can decorrelate, so the framework owns correlated {@code IN}
 * end-to-end (rather than relying on the legacy {@code TransformCorrelatedInPredicateToJoin}).
 * Active only when `decorrelation_strategy = UNIFIED`.
 *
 * <p>The three-valued {@code x IN (subquery)} result is computed per outer row from two counts over
 * the subquery rows {@code b}:
 * <ul>
 *     <li>{@code countMatches}     = count of rows where {@code x = b} (a {@code NULL} on either
 *         side makes the comparison {@code NULL}, which the {@code FILTER} excludes);</li>
 *     <li>{@code countNullMatches} = count of rows where {@code x IS NULL OR b IS NULL}.</li>
 * </ul>
 * Then {@code IN = CASE WHEN countMatches > 0 THEN true WHEN countNullMatches > 0 THEN null ELSE
 * false}. An empty subquery yields both counts {@code 0} → {@code false} ({@code x IN ()} is
 * {@code false}, even for {@code NULL} {@code x}), which the framework's scalar global-aggregate
 * path produces via its {@code non_null} mask. {@code NOT IN} is the surrounding {@code NOT} over
