        ...
        implements AutoCloseable
{
    ...
    public void taskFinished(TaskId taskId, boolean successful, long finalContributionSequence)
    {
        ...
        List<Completion> completions = new ArrayList<>();
        synchronized (this) {
            if (closed || !acceptsTaskAttempt(taskId.attemptId())) {
                return;
            }
            if (!successful) {
        ...
        for (GroupContext group : groups.values()) {
            ...
            if (successfulContributions.isEmpty() && !group.committedPartitions.contains(producerPartition)) {
                commitLocked(
                        group,
                        producerPartition,
                        new RuntimeConstraintContribution(
                                group.spec.groupId(),
                                dynamicBindingId(group.spec.originPlanNodeId()),
                                taskId.partitionId(),
                                taskId.attemptId(),
                                1,
                                new RuntimeMembershipPayload(
                                        group.spec.collectedLanes().stream().map(lane -> Domain.none(lane.type())).toList(),
                                        RuntimeConstraintNullMatchMode.ORDINARY,
                                        false,
                                        group.spec.collectedLanes().stream().map(_ -> false).toList())),
                        completions);
            }
            else {
    ...
    private RuntimeMembershipPayload mergePayloads(RuntimeMembershipPayload current, RuntimeMembershipPayload update)
    {
        ...
        checkArgument(current.scalarDomains().size() == update.scalarDomains().size(), "membership contributions have different lane counts");
        ...
        return new RuntimeMembershipPayload(
                IntStream.range(0, current.scalarDomains().size())
                        .mapToObj(lane -> boundDomain(current.scalarDomains().get(lane).union(update.scalarDomains().get(lane))))
                        .toList(),
                current.nullMatchMode(),
                current.sawInputRow() || update.sawInputRow(),
                IntStream.range(0, current.sawNulls().size())
                        .mapToObj(lane -> current.sawNulls().get(lane) || update.sawNulls().get(lane))
                        .toList());
    }
    ...
    private RuntimeConstraintPayload boundPayload(RuntimeConstraintPayload payload)
    {
        ...
        return new RuntimeMembershipPayload(
                membership.scalarDomains().stream().map(this::boundDomain).toList(),
