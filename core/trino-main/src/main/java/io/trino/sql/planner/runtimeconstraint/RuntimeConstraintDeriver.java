            ...
            List<RuntimeConstraintPayload> contributions)
    {
        ...
        if (contributions.isEmpty()) {
            merged = new RuntimeMembershipPayload(
                    group.collectedLanes().stream().map(lane -> Domain.none(lane.type())).toList(),
                    ORDINARY,
                    false,
                    group.collectedLanes().stream().map(_ -> false).toList());
        }
        else {
            ...
            checkArgument(payloads.stream().allMatch(payload -> payload.scalarDomains().size() == laneCount), "membership contribution has wrong lane count");
            ...
            merged = new RuntimeMembershipPayload(
                    IntStream.range(0, laneCount)
                            .mapToObj(lane -> Domain.union(payloads.stream().map(payload -> payload.scalarDomains().get(lane)).toList()))
                            .toList(),
                    payloads.getFirst().nullMatchMode(),
                    payloads.stream().anyMatch(RuntimeMembershipPayload::sawInputRow),
                    IntStream.range(0, laneCount)
                            .mapToObj(lane -> payloads.stream().anyMatch(payload -> payload.sawNulls().get(lane)))
                            .toList());
        }
        ...
        for (RuntimeConstraintDerivation derivation : group.derivations()) {
            RuntimeMembershipPayload selected = new RuntimeMembershipPayload(
                    derivation.collectedLaneIndexes().stream().map(merged.scalarDomains()::get).toList(),
                    merged.nullMatchMode(),
                    merged.sawInputRow(),
                    derivation.collectedLaneIndexes().stream().map(merged.sawNulls()::get).toList());
            List<Domain> derivedDomains = new ArrayList<>();
            for (int lane = 0; lane < selected.scalarDomains().size(); lane++) {
                derivedDomains.add(applyComparison(selected.scalarDomains().get(lane), derivation.operator(), derivation.nullAllowed(), selected.sawInputRow(), selected.sawNulls().get(lane)));
            }
            derivedPayloads.put(derivation.constraintId(), new RuntimeMembershipPayload(
                    derivedDomains,
                    derivation.nullAllowed() ? NULL_SAFE : ORDINARY,
                    selected.sawInputRow(),
                    selected.sawNulls()));
        }
