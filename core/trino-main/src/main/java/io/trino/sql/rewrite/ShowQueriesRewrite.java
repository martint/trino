            ...
            extends AstVisitor<Node, Void>
    {
        ...
        protected Node visitShowTables(ShowTables showTables, Void context)
        {
            ...
            if (likePattern.isPresent()) {
                Expression likePredicate = new Predicated(identifier("table_name"), new LikePredicate(false, new StringLiteral(likePattern.get()), showTables.getEscape().map(StringLiteral::new)));
        ...
        protected Node visitShowSchemas(ShowSchemas node, Void context)
        {
            ...
            if (likePattern.isPresent()) {
                predicate = Optional.of(new Predicated(identifier("schema_name"), new LikePredicate(false, new StringLiteral(likePattern.get()), node.getEscape().map(StringLiteral::new))));
            }
        ...
        protected Node visitShowCatalogs(ShowCatalogs node, Void context)
        {
            ...
            if (rows.isEmpty()) {
                ...
                predicate = Optional.of(new Predicated(identifier("catalog"), new LikePredicate(false, new StringLiteral(node.getLikePattern().get()), node.getEscape().map(StringLiteral::new))));
            }
        ...
        protected Node visitShowColumns(ShowColumns showColumns, Void context)
        {
            ...
            if (likePattern.isPresent()) {
                Expression likePredicate = new Predicated(identifier("column_name"), new LikePredicate(false, new StringLiteral(likePattern.get()), showColumns.getEscape().map(StringLiteral::new)));
                predicate = logicalAnd(predicate, likePredicate);
            }
        ...
        protected Node visitShowFunctions(ShowFunctions node, Void context)
        {
            ...
            return simpleQuery(
                    ...
                    node.getLikePattern()
                            .map(like -> new Predicated(identifier("function_name"), new LikePredicate(false, new StringLiteral(like), node.getEscape().map(StringLiteral::new))))
        ...
        protected Node visitShowSession(ShowSession node, Void context)
        {
            ...
            Optional<String> likePattern = node.getLikePattern();
            if (likePattern.isPresent()) {
                predicate = and(predicate, new Predicated(identifier("name"), new LikePredicate(false, new StringLiteral(likePattern.get()), node.getEscape().map(StringLiteral::new))));
            }
