...
/**
 * Generates bytecode for {@link Match}: evaluate the operand once into a temp variable, then test
 * each clause's lambda body with the parameter resolved to that temp. Single-evaluation of the
 * operand is the whole reason this node exists separately from a searched {@link io.trino.sql.ir.Case};
 * naively substituting the operand into each clause's predicate would re-evaluate it N times and
 * change semantics for non-deterministic operands such as {@code random()}.
 * <p>
 * For SQL three-valued logic, the operand's null state is captured into a separate flag at
 * evaluation time and restored to {@code wasNull} before each clause's predicate body runs, so
 * {@code WHEN IS NULL} and other null-aware predicates see the operand as null. The temp variable
 * holds the Java default when the operand was null; predicates that read it should already gate on
 * {@code wasNull} (the IR codegen convention) and will produce the right result.
 */
        ...
        implements BytecodeGenerator
{
    ...
    /**
     * Return the clause's lambda body with the operand parameter substituted by {@code tempReference}
