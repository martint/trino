...
public final class NestedLoopRuntimeConstraintSource
{
    ...
    private synchronized void register(RuntimeConstraintWiringContext context)
    {
        ...
        context.registerSource(
                ...
                completionPolicy,
                bindings.stream().map(Binding::operator).toList(),
                bindings.stream().map(Binding::nullAllowed).toList(),
                bindings.stream().map(binding -> channels.indexOf(binding.buildChannel())).toList());
        initializeConsumer();
    }
    ...
    private void initializeConsumer()
    {
        ...
        consumer = new LocalRuntimeConstraintConsumer(
                channels,
                channels.stream().map(buildTypes::get).toList(),
                (domains, observation) -> manager.get().addContribution(
                        sourceId,
    ...
    private List<RuntimeConstraintId> constraintIds()
    {
