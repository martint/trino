        ...
        implements AutoCloseable
{
    ...
    {
        return snapshotRetainedBytes + contributions.getRetainedBytes();
    }
    ...
    public synchronized void registerConsumers(List<RuntimeConstraintWiringReport.Binding> bindings)
    {
        for (RuntimeConstraintWiringReport.Binding binding : bindings) {
            if (consumerIds.add(binding.constraintId())) {
                snapshots.put(binding.constraintId(), RuntimeConstraintSnapshot.pending(binding.constraintId(), generation));
            }
        }
    }
    ...
    public synchronized void registerSource(RuntimeConstraintWiringReport.Source source)
    {
        ...
        RuntimeConstraintProducerGroup group = new RuntimeConstraintProducerGroup(
                ...
                IntStream.range(0, source.types().size())
                        ...
                        .toList(),
                IntStream.range(0, source.constraintIds().size())
                        .mapToObj(index -> new RuntimeConstraintDerivation(
                                source.constraintIds().get(index),
                                RuntimeConstraintKind.MEMBERSHIP,
                                ImmutableList.of(source.collectedLaneIndexes().get(index)),
                                source.operators().get(index),
                                source.nullAllowed().get(index)))
                        .toList(),
                source.replicated());
    ...
    {
        memoryContext.setBytes(snapshotRetainedBytes + contributions.getRetainedBytes());
    }
    ...
    private RuntimeConstraintSnapshot admitSnapshot(RuntimeConstraintSnapshot current, RuntimeConstraintSnapshot candidate)
    {
        ...
        long totalRetainedBytes = candidateRetainedBytes + contributions.getRetainedBytes();
        ...
        checkState(trySetMemoryBytes(disabledRetainedBytes + contributions.getRetainedBytes()), "failed to reserve memory for disabled runtime constraint snapshot");
    ...
    {
        return trySetMemoryBytes(snapshotRetainedBytes + contributions.getRetainedBytes());
    }
    ...
    private static RuntimeConstraintPayload unrestricted(RuntimeConstraintPayload payload)
    {
        ...
        return new RuntimeMembershipPayload(
                membership.scalarDomains().stream()
                        .map(domain -> Domain.all(domain.getType()))
                        .toList(),
