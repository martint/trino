        ...
        extends SqlScalarFunction
{
    ...
    @UsedByGeneratedCode
    public static JsonPayload jsonArray(RowType elementsRowType, SqlRow elementsRow, boolean nullOnNull)
    {
        return JsonPayload.of(JsonType.jsonValue(buildArray(elementsRowType, elementsRow, nullOnNull)));
    }
    ...
    private static JsonValue buildArray(RowType elementsRowType, SqlRow elementsRow, boolean nullOnNull)
    {
        ...
        for (int i = 0; i < elementsRowType.getFields().size(); i++) {
            ...
            if (element == null) {
                ...
                Slice payload = element instanceof JsonPayload jsonValue
                        ? jsonValue.payload()
                        : (Slice) element;
