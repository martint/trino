...
public class ScalarHeader
{
    ...
    public static List<ScalarHeader> fromAnnotatedElement(AnnotatedElement annotated)
    {
        ...
        if (scalarFunction != null) {
            checkArgument(staticMethod == null || instanceMethod == null, "@StaticMethod and @InstanceMethod are mutually exclusive on " + annotated);
            String baseName = scalarFunction.value().isEmpty() ? camelToSnake(annotatedName(annotated)) : scalarFunction.value();
            Optional<TypeSignature> receiverType = staticMethod == null ? Optional.empty() : Optional.of(parseTypeSignature(staticMethod.value(), ImmutableSet.of()));
