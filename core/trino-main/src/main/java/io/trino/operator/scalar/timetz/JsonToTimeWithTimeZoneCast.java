...
public final class JsonToTimeWithTimeZoneCast
{
    ...
    @LiteralParameters("p")
    @SqlType("time(p) with time zone")
    public static LongTimeWithTimeZone castToLong(@LiteralParameter("p") long precision, ConnectorSession session, @SqlType(JSON) JsonPayload json)
    {
        Slice text = textFromJson(json);
    ...
    private static Slice textFromJson(JsonPayload json)
    {
        ...
        return switch (type) {
            ...
            case TimeWithTimeZoneType timeType -> timeType.isShort()
            ...
            default -> throw new TrinoException(INVALID_CAST_ARGUMENT, format("Cannot cast SQL/JSON value of type %s to time with time zone", type));
        };
    }
}
