...
public final class TimestampToJsonCast
{
    ...
    @LiteralParameters("p")
    @SqlType(JSON)
    public static JsonValue cast(@LiteralParameter("p") long precision, ConnectorSession session, @SqlType("timestamp(p)") long timestamp)
    {
    ...
    @SqlType(JSON)
    public static JsonValue cast(@LiteralParameter("p") long precision, ConnectorSession session, @SqlType("timestamp(p)") LongTimestamp timestamp)
    {
        return toJson(formatTimestamp((int) precision, timestamp.getEpochMicros(), timestamp.getPicosOfMicro(), UTC, TIMESTAMP_FORMATTER));
    }

    private static JsonValue toJson(String formatted)
    {
        try {
