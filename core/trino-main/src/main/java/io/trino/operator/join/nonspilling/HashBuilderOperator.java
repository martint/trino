        ...
        implements Operator
{
            ...
            implements OperatorFactory
    {
        ...
        public void completeRuntimeConstraintWiring(RuntimeConstraintWiringContext context)
        {
            ...
            if (!channels.isEmpty()) {
                List<Integer> constraintChannels = runtimeConstraintBuildChannels();
                List<ComparisonOperator> operators = Stream.concat(
                                hashChannels.stream().map(_ -> ComparisonOperator.EQUAL),
                                filterFunctionFactory.stream()
                                        .flatMap(factory -> factory.getRuntimeConstraintComparisons().stream())
                                        .map(RuntimeConstraintComparison::operator))
                        .toList();
                List<Boolean> nullAllowed = Stream.concat(
                                hashChannels.stream().map(_ -> false),
                                filterFunctionFactory.stream()
                                        .flatMap(factory -> factory.getRuntimeConstraintComparisons().stream())
                                        .map(RuntimeConstraintComparison::nullAllowed))
                        .toList();
                context.registerSource(
                        planNodeId,
                        IntStream.range(0, constraintChannels.size()).mapToObj(index -> RuntimeConstraintRequest.joinConstraintId(planNodeId, index)).toList(),
                        channels.stream().map(buildTypes::get).toList(),
                        runtimeConstraintCompletionPolicy,
                        operators,
                        nullAllowed,
                        constraintChannels.stream().map(channels::indexOf).toList());
                initializeRuntimeConstraintSource();
            }
        }
        ...
        public List<RuntimeConstraintRequest> getInputRuntimeConstraints(RuntimeConstraintWiringContext context)
        {
            ...
            List<Type> buildTypes = lookupSourceFactoryManager.getJoinBridge().getTypes();
            List<Integer> channels = runtimeConstraintBuildChannels();
            List<ComparisonOperator> operators = Stream.concat(
                            hashChannels.stream().map(_ -> ComparisonOperator.EQUAL),
                            filterFunctionFactory.stream()
                                    .flatMap(factory -> factory.getRuntimeConstraintComparisons().stream())
            ...
            return IntStream.range(0, channels.size())
                    .mapToObj(index -> RuntimeConstraintRequest.collection(
                            RuntimeConstraintRequest.joinConstraintId(planNodeId, index),
                            channels.get(index),
                            operators.get(index),
                            nullAllowed.get(index),
                            buildTypes.get(channels.get(index)),
        ...
        {
            return runtimeConstraintBuildChannels().stream().distinct().toList();
        }

        private List<Integer> runtimeConstraintBuildChannels()
        {
            return Stream.concat(
                            hashChannels.stream(),
                            filterFunctionFactory.stream()
                                    .flatMap(factory -> factory.getRuntimeConstraintComparisons().stream())
                                    .map(RuntimeConstraintComparison::buildChannel))
                    .toList();
        }
        ...
        private void initializeRuntimeConstraintSource()
        {
            ...
            runtimeConstraintConsumer = new LocalRuntimeConstraintConsumer(
                    channels,
                    channels.stream().map(buildTypes::get).toList(),
                    (domains, observation) -> runtimeConstraintManager.get().addContribution(
                            planNodeId,
