...
public final class JsonToTimestampWithTimeZoneCast
{
    ...
    @LiteralParameters("p")
    @SqlType("timestamp(p) with time zone")
    public static LongTimestampWithTimeZone castToLong(@LiteralParameter("p") long precision, ConnectorSession session, @SqlType(JSON) JsonValue json)
    {
        Slice text = textFromJson(json);
    ...
    private static Slice textFromJson(JsonValue json)
    {
        ...
        throw new TrinoException(INVALID_CAST_ARGUMENT, format("Cannot cast SQL/JSON value of type %s to timestamp with time zone", type));
    }
}
