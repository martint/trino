        ...
        implements VariableWidthType
{
    ...
    private JsonType()
    {
        super(new TypeSignature(NAME), JsonPayload.class, JsonBlock.class);
    }
    ...
    public Object getObject(Block block, int position)
    {
        ...
        return JsonPayload.of(payloadSlice(block, position));
    }
    ...
    {
        return JsonPayload.of(getFlatPayload(fixedSizeSlice, fixedSizeOffset, variableSizeSlice, variableSizeOffset));
    }
    ...
    @ScalarOperator(EQUAL)
    private static boolean equalOperator(JsonPayload left, @BlockPosition ValueBlock rightBlock, @BlockIndex int rightPosition)
    {
        return slicesEqualOrSemanticallyEqual(left.payload(), payloadSlice(rightBlock, rightPosition));
    }

    @ScalarOperator(EQUAL)
    private static boolean equalOperator(@BlockPosition ValueBlock leftBlock, @BlockIndex int leftPosition, JsonPayload right)
    {
    ...
    /// Decodes a {@link JsonType} payload to a {@link JsonItem}. The
    /// JSON_ERROR sentinel is returned as {@link JsonInputError#JSON_ERROR}; every
    /// other payload yields a {@link JsonValueView} over the stored bytes.
    public static JsonItem toPathItem(Slice slice)
    {
        if (slice == null) {
            return null;
        }
