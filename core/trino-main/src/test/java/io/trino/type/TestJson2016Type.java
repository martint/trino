...
class TestJson2016Type
{
    ...
    {
        MaterializedJsonValue duplicateObject = parseJson("{\"key\":1,\"key\":2}");
        MaterializedJsonValue numericArray = new JsonArrayItem(List.of(
        ...
        assertThat(JsonItems.asJsonValue((io.trino.json.JsonPathItem) JSON_2016.getObject(block, 0))).isEqualTo(JsonItems.asJsonValue(duplicateObject));
        assertThat(JsonItems.asJsonValue((io.trino.json.JsonPathItem) JSON_2016.getObject(block, 1))).isEqualTo(JsonItems.asJsonValue(numericArray));
        ...
        assertThat(JsonItems.asJsonValue((io.trino.json.JsonPathItem) JSON_2016.getObject(region, 0))).isEqualTo(JsonItems.asJsonValue(numericArray));
    ...
    void testNumericTypeWidthsSurviveRoundTrip()
    {
        ...
        assertThat(JsonItems.asJsonValue((io.trino.json.JsonPathItem) JSON_2016.getObject(block, 0))).isEqualTo(item);
    }

    @Test
    void testWriteObjectAcceptsJsonValueView()
    {
        MaterializedJsonValue item = parseJson("{\"key\":[1,2,3]}");
        ...
        JsonValueView view = JsonValueView.fromObject((io.trino.json.JsonPathItem) JSON_2016.getObject(sourceBlock, 0)).orElseThrow();
        ...
        assertThat(JsonItems.asJsonValue((io.trino.json.JsonPathItem) JSON_2016.getObject(targetBlock, 0))).isEqualTo(JsonItems.asJsonValue(item));
    }
    ...
    void testJsonValueViewEqualityIsSemantic()
    {
        ...
        MaterializedJsonValue value = parseJson("{\"x\":[1,null,\"abc\"]}");

        BlockBuilder leftBuilder = JSON_2016.createBlockBuilder(null, 1);
        JSON_2016.writeObject(leftBuilder, value);
        JsonValueView left = JsonValueView.fromObject((JsonPathItem) JSON_2016.getObject(leftBuilder.buildValueBlock(), 0)).orElseThrow();

        BlockBuilder rightBuilder = JSON_2016.createBlockBuilder(null, 1);
        JSON_2016.writeObject(rightBuilder, parseJson("{\"x\":[1,null,\"abc\"]}"));
        JsonValueView right = JsonValueView.fromObject((JsonPathItem) JSON_2016.getObject(rightBuilder.buildValueBlock(), 0)).orElseThrow();
    ...
    private static MaterializedJsonValue parseValue(String json)
    {
        MaterializedJsonValue item = parseJson(json);
