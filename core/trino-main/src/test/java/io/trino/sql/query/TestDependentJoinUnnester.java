 ...
 * <p>
 * All queries here run with `decorrelation_strategy = UNIFIED`. The framework should produce
...
public class TestDependentJoinUnnester
{
    ...
    private Session unifiedSession()
    {
        return assertions.sessionBuilder()
                .setSystemProperty(DECORRELATION_STRATEGY, UNIFIED.name())
                .build();
    }

    @Test
    public void testScalarSubqueryReturningBareCorrelationDefersToLegacy()
    {
        ...
        // (e.g. [1, 1] here) instead of NULL for the non-matching row. The framework must defer; legacy
        // also rejects this shape. Regression test for the bare-correlation-scalar-output bug.
    ...
    @Test
    public void testQuantifiedComparisonOverCorrelationDefersToLegacy()
    {
        ...
        // later optimizer rule rejects, so the framework must defer; legacy also rejects this shape.
        assertThat(assertions.query(
                unifiedSession(),
    ...
    @Test
    public void testCorrelationCarriedAsStructuralOutputDefersToLegacy()
    {
        ...
        // framework must defer rather than emit a malformed join; legacy also rejects this shape, so
    ...
    public void testCorrelatedFilterEquality()
    {
        ...
        assertThat(assertions.query(unifiedSession(),
    ...
    public void testCorrelatedGroupedAggregationInsideExists()
    {
        ...
        // position would hit the COUNT-bug and is deliberately left to legacy — see
        // DependentJoinUnnester.unnest.)
        assertThat(assertions.query(unifiedSession(),
                "SELECT a FROM (VALUES 1, 5, 10) t(a) " +
    ...
    {
        // ⊕ pushdown — INTERSECT (distinct) partitioned by outer row via the input columns in the
        // set-op key.
    ...
    {
        // ⊕ pushdown — EXCEPT (distinct), again keyed by the outer row.
    ...
    public void testCorrelatedUnnestInsideLateralWithFilter()
    {
        ...
        // framework's visitUnnest. Legacy DecorrelateUnnest rejects the correlated filter on the
    ...
    public void testCorrelatedGroupingSetsInLateral()
    {
        ...
        // T₁'s columns are added to every grouping set, computing the sets per outer row.
        //   t=1: u.k=1 → {(1,10),(1,20)} → set (u.k): k=1 cnt=2; set (): k=NULL cnt=2
        //   t=2: u.k=2 → {(2,30)}        → set (u.k): k=2 cnt=1; set (): k=NULL cnt=1
        //   t=5: no match → empty source → no rows (INNER lateral drops t=5)
        assertThat(assertions.query(unifiedSession(),
                "SELECT t.a, l.k, l.cnt FROM (VALUES 1, 2, 5) t(a), LATERAL (" +
                        "  SELECT u.k AS k, count(*) AS cnt FROM (VALUES (1, 10), (1, 20), (2, 30)) u(k, b) " +
                        "  WHERE u.k = t.a GROUP BY GROUPING SETS ((u.k), ())) l(k, cnt)"))
                .matches("VALUES (1, 1, BIGINT '2'), (1, CAST(NULL AS INTEGER), BIGINT '2'), " +
                        "(2, 2, BIGINT '1'), (2, CAST(NULL AS INTEGER), BIGINT '1')");
    }
