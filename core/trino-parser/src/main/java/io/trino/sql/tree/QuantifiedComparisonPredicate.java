...
/**
 * SQL spec {@code <quantified comparison predicate part 2> ::= <comp op> <quantifier> ( <table subquery> )}.
 * The quantifier ({@code ALL}, {@code ANY}, {@code SOME}) lives alongside the comparison operator
 * (without the {@link ComparisonPredicate} class's flip/negate machinery, which doesn't lift to
 * quantified form).
 */
        ...
        extends Predicate
{
    ...
    @Deprecated
    public QuantifiedComparisonPredicate(ComparisonPredicate.Operator operator, Quantifier quantifier, Expression subquery)
    {
        this(null, operator, quantifier, subquery);
    }
