...
/**
 * SQL spec {@code <character like predicate part 2> ::= [NOT] LIKE <character pattern> [ESCAPE <escape character>]}.
 * The in-place {@code NOT LIKE} is recorded via {@link #isNegated()}; outer {@code NOT (x LIKE y)}
 * stays as a {@link NotExpression} wrapping a non-negated {@code LikePredicate}.
 */
        ...
        extends Predicate
{
    ...
    @Deprecated
    public LikePredicate(boolean negated, Expression pattern, Optional<Expression> escape)
    {
        this(null, negated, pattern, escape);
    }
